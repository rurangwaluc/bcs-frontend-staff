...
/**
 * Backend-aligned endpoints (based on your Fastify prefixes)
 *
 ...
 *
 * ✅ Expenses (prefix: /cash/expenses)  (route file usually uses "/")
 *  - GET  /cash/expenses
 *  - POST /cash/expenses
 *
 * ⚠️ Cash Reconcile: your routes currently define "/cash/reconcile" and "/cash/reconciles"
 *    but app.js registers prefix "/cash-reconcile".
 *    So the REAL paths become:
 *      - GET  /cash-reconcile/cash/reconciles
 *      - POST /cash-reconcile/cash/reconcile
 *    (Yes, it's weird, but this UI matches what you registered.)
 *
 * ✅ Payments
 *  - GET  /payments
...
const ENDPOINTS = {
  ...
  // Cash reconcile (matches your prefix + route paths)
...
export default function CashierPage() {
  ...
  const loadReconciles = useCallback(async () => {
    ...
    try {
      ...
      // if your reconcile route is different, you'll paste the 404 and we will adjust ENDPOINTS
  ...
  useEffect(() => {
    ...
    // always keep sessions available (banner)
    loadSessions();
  ...
  async function recordPayment(e) {
    ...
    // Optional guard: require open session to record anything
    ...
    try {
      ...
      await loadSummary();
      await loadPayments();
      await loadSessions(); // session closing balance is separate, but keep it fresh
    } catch (e2) {
      ...
      await apiFetch(ENDPOINTS.CASHBOOK_DEPOSIT_CREATE, {
        ...
        body: {
          ...
          note: depositNote?.trim()
            ...
            : undefined,
          // if backend expects sessionId, we add it safely
  ...
  async function createReconcile(e) {
    ...
    const payload = {
      ...
      cashierId: me.id,
      expectedCash: Number(
        currentOpenSession.closingBalance ??
        ...
        {tab === "reconcile" ? (
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
            <div className="bg-white rounded-xl shadow p-4">
              ...
              <div className="text-xs text-gray-500 mt-1">
                POST {ENDPOINTS.CASH_RECONCILE_CREATE} (requires open session)
              </div>
              ...
              <div className="mt-3 text-xs text-gray-500">
                If this tab returns 404, paste the error and we will adjust ONLY
                the reconcile endpoints.
              </div>
            </div>
            ...
            <div className="bg-white rounded-xl shadow overflow-hidden">
              <div className="p-4 border-b flex items-center justify-between">
                <div>
                  ...
                  <div className="text-xs text-gray-500 mt-1">
                    GET {ENDPOINTS.CASH_RECONCILES_LIST}
                  </div>
                </div>
                <button
